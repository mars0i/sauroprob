...
(defn three-plots
  "Displays a plot of a functionm, possibly with iteration lines; a plot of
  ...
  [& {:keys [x-max  ; max x val in function plot (also max y)
             f      ; function to plot, iterate
             init-x ; initial x val for iterations
  ...
  (let [iterates (iterate f init-x)]
    (kind/fragment [(kind/md ["Plot of the function, with sample iterations beginning from " init-x ":"])
                    (-> (tc/concat (sp/iter-lines init-x n-iter-lines :fun "iteration" f)
