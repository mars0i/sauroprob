...
(defn plots-grid
  "Displays a a cobweb plot--a plot of a function and
  compositions of itself with itself, possibly with iteration
  lines; a plot of values against (discrete) time generated by
  iteration; a histogram of those values, and a corresponding
  empirical CDF. See comments in code for details about args.  All
  functions in fs should take exaclty one argument.  The function
  used to iterate should be the first element in fs.
  Parameters:
  x-max:  max x val in function plot (also max y)
  fs:     functions to plot
  labels: labels corresponding to functions
  ...
  n-seq-iterates:  how many iterations in plot of iteration values
  n-dist-iterates:  how many iterations in histogram?
  intro-label-md: informational Markdown string displayed above the plots"
  [& {:keys [x-min
             x-max  ; max x val in function plot (also max y)
             fs     ; functions to plot
             labels ; labels corresponding to functions
